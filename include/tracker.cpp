...
cv::Mat acme::Tracker::DrawBoxes(cv::Mat frame,
  ...
  for (int i = 0; i < pixels; ++i) {
    ...
    if (confidence >= CONFIDENCE_THRESHOLD) {
      float *classes_scores = data + 5; // Class scores start from index 5 for each detection.
      ...
      if (max_class_score > SCORE_THRESHOLD) {
        ...
        bounding_boxes.push_back(cv::Rect(box_left, box_top, box_width, box_height));
      }
    }
  ...
  cv::dnn::NMSBoxes(bounding_boxes, confidence_list, SCORE_THRESHOLD, NMS_THRESHOLD,
                    nms_boxes);
  for (unsigned int i = 0; i < nms_boxes.size(); i++) {
    ...
    rectangle(frame, cv::Point(box_left, box_top),
              cv::Point(box_left + box_width, box_top + box_height), cv::Scalar(0, 255, 0), 3);

    std::string label; // = cv::format("%.2f", confidence_list[idx]);
    label = person[class_ids[idx]] + ":" + cv::format("%.2f", confidence_list[idx]);
